...
/**
 * @brief 计算点云法线并生成带法线的点云
 * Compute normals and generate point cloud with normals
 */
                       ...
                       pcl::PointCloud<pcl::PointNormal>::Ptr &cloud_normals)
{
    ...
    n.setNumberOfThreads(8); // 使用多线程加速法线计算
    n.setInputCloud(cloud);  // 输入点云
    n.setSearchMethod(tree); // KD树搜索
    n.setKSearch(10);        // 设置K近邻点数
...
int main()
{
    ...
    // -----------------------------加载源点云数据-----------------------------
    if (pcl::io::loadPCDFile<pcl::PointXYZ>("source_cloud.pcd", *source_cloud) < 0)
    {
    ...
    // ----------------------------加载目标点云数据----------------------------
    if (pcl::io::loadPCDFile<pcl::PointXYZ>("target_cloud.pcd", *target_cloud) < 0)
    {
        std::cerr << "Error: Could not read target point cloud file!" << std::endl;
        return -1;
    }

    std::cout << "=== 4PCS + Symmetric Point-to-Plane ICP Registration ===" << std::endl;
    ...
    kfpcs.setNumberOfThreads(12);   // 使用12个线程加速
    ...
    std::cout << "NumberOfThreads=12, Lambda=0.3" << std::endl;
    ...
    // 计算法线
    std::cout << "\n=== Step 2: Computing Normals for Symmetric Point-to-Plane ICP ===" << std::endl;
    ...
    // 对称点到平面ICP精配准
    std::cout << "\n=== Step 3: Performing Symmetric Point-to-Plane ICP Fine Registration ===" << std::endl;
    ...
    {
        std::cerr << "\nSymmetric Point-to-Plane ICP registration failed!" << std::endl;
        return -1;
    }

    std::cout << "\nSymmetric ICP has converged, score is " << symm_icp.getFitnessScore() << std::endl;
    Eigen::Matrix4f final_transformation = symm_icp.getFinalTransformation();
    std::cout << "Symmetric Point-to-Plane ICP transformation matrix:" << std::endl;
    std::cout << final_transformation << std::endl;

    // 计算总变换矩阵（4PCS + 对称点到平面ICP）
    Eigen::Matrix4f total_transform = symm_icp.getFinalTransformation();
    ...
    std::cout << "Symmetric Point-to-Plane ICP time: " << duration_fine.count() << " ms" << std::endl;
    std::cout << "Final fitness score: " << symm_icp.getFitnessScore() << std::endl;

    // 质量评估
    if (symm_icp.getFitnessScore() < 0.001f)
    {
        std::cout << "Registration quality: Excellent" << std::endl;
    }
    else if (symm_icp.getFitnessScore() < 0.005f)
    {
        std::cout << "Registration quality: Very Good" << std::endl;
    }
    else if (symm_icp.getFitnessScore() < 0.01f)
    {
        std::cout << "Registration quality: Good" << std::endl;
    }
    else if (symm_icp.getFitnessScore() < 0.05f)
    {
    ...
    // 保存配准结果
    pcl::io::savePCDFileASCII("aligned_cloud.pcd", *final_aligned_cloud);
    std::cout << "\nRegistration result saved to aligned_cloud.pcd" << std::endl;

    // 可视化配准结果
    pcl::visualization::PCLVisualizer::Ptr viewer(new pcl::visualization::PCLVisualizer("4PCS + Symmetric Point-to-Plane ICP Registration Results"));
    ...
    // 添加文本说明
    viewer->addText("Symmetric Point-to-Plane ICP", 10, 20, 16, 1, 1, 1, "fine_text", v3);
    ...
    std::string final_score = "ICP Score: " + std::to_string(symm_icp.getFitnessScore());
    ...
    std::string registration_info = "4PCS + Symmetric ICP";
    viewer->addText(total_time, 10, 80, 14, 1, 1, 1, "time_text", v4);
    viewer->addText(registration_info, 10, 100, 14, 1, 1, 1, "reg_info_text", v4);

    // 公共设置
    viewer->addText("4PCS Coarse + Symmetric Point-to-Plane ICP Fine Registration", 250, 20, 18, 1, 1, 1, "title");
    ...
    std::cout << "3rd View: Symmetric Point-to-Plane ICP result (Green: Final aligned)" << std::endl;
