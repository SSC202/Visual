...
int main()
{
    ...
    // 加载源点云数据
    if (pcl::io::loadPCDFile<pcl::PointXYZ>("E://data//1.pcd", *source_cloud) < 0)
    {
    ...
    // 加载目标点云数据
    if (pcl::io::loadPCDFile<pcl::PointXYZ>("E://data//2.pcd", *target_cloud) < 0)
    {
    ...
    std::cout << "4PCS Parameters: ApproxOverlap=0.55, Delta=0.05, NumberOfSamples=200" << std::endl;
    ...
    p_icp.setInputTarget(target_with_normals); // 目标点云（带法线）
    p_icp.setTransformationEpsilon(1e-10);     // 设置转换终止条件
    p_icp.setMaxCorrespondenceDistance(0.5);   // 设置最大对应距离
