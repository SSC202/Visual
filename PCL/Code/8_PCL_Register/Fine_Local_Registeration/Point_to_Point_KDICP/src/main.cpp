...
int main(int argc, char **argv)
{
    ...
    // 读取源点云和目标点云
    if (pcl::io::loadPCDFile<pcl::PointXYZ>("1.pcd", *source_cloud) == -1)
    {
        std::cerr << "Error: Could not read source point cloud file!" << std::endl;
        return -1;
    }
    if (pcl::io::loadPCDFile<pcl::PointXYZ>("2.pcd", *target_cloud) == -1)
    {
    ...
    std::cout << "NumberOfThreads=8, Lambda=0.3" << std::endl;
    ...
    tree1->setInputCloud(coarse_aligned_cloud);
    ...
    icp.setSearchMethodSource(tree1);         // 源点云KD树搜索方法
    icp.setSearchMethodTarget(tree2);         // 目标点云KD树搜索方法
    icp.setTransformationEpsilon(1e-10);      // 为终止条件设置最小转换差异
    icp.setMaxCorrespondenceDistance(0.01);    // 设置对应点对之间的最大距离
    icp.setEuclideanFitnessEpsilon(1e-10);     // 设置收敛条件是均方误差和小于阈值
    icp.setMaximumIterations(50);             // 最大迭代次数

    std::cout << "ICP Parameters: MaxCorrespondenceDistance=0.01, EuclideanFitnessEpsilon=1e-10" << std::endl;
    std::cout << "MaximumIterations=50, TransformationEpsilon=1e-10" << std::endl;

    // 执行ICP配准（使用4PCS的结果作为初始变换）
    auto start_time_fine = std::chrono::steady_clock::now();
    icp.align(*final_aligned_cloud, kfpcs.getFinalTransformation());
    ...
    std::cout << "Applied " << icp.nr_iterations_ << " ICP iterations in " << duration_fine.count() << " ms" << std::endl;
    ...
    else if (icp.getFitnessScore() < 0.1f)
    {
    ...
    viewer->addText("White: Target", 10, 40, 14, 1, 1, 1, "target_text_v3", v3);
    ...
    std::string iteration_info = "Iterations: " + std::to_string(icp.nr_iterations_);
    viewer->addText(final_score, 10, 80, 14, 1, 1, 1, "final_score_text", v3);
    viewer->addText(total_time, 10, 100, 14, 1, 1, 1, "time_text", v3);
    viewer->addText(iteration_info, 10, 120, 14, 1, 1, 1, "iteration_text", v3);

    // 公共设置
    viewer->addText("4PCS + ICP Point Cloud Registration", 300, 20, 18, 1, 1, 1, "title");
    ...
    std::cout << "Left: Original clouds (Red: Source, Green: Target)" << std::endl;
    std::cout << "Middle: 4PCS coarse result (Yellow: Coarse aligned)" << std::endl;
    std::cout << "Right: ICP fine result (White: Target, Blue: Final aligned)" << std::endl;
