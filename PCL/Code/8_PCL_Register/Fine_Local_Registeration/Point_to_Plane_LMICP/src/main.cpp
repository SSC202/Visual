...
/**
 * @brief 计算点云法线并将法线信息拼接到点云数据中
 * Compute normals for point cloud and concatenate with points
 */
                       ...
                       pcl::PointCloud<pcl::PointNormal>::Ptr &cloud_normals)
{
    ...
    n.setKSearch(1);     // 点云法向计算时，需要搜索的近邻点大小
...
int main()
{
    ...
    std::cout << "NumberOfThreads=10, Lambda=0.3" << std::endl;
    ...
    // 检查ICP配准是否成功
    if (!lm_icp.hasConverged())
    {
        std::cerr << "\nPoint-to-Plane ICP registration failed!" << std::endl;
        return -1;
    }

    std::cout << "\nICP has converged, score is " << lm_icp.getFitnessScore() << std::endl;
    Eigen::Matrix4f icp_transformation = lm_icp.getFinalTransformation();
    std::cout << "Point-to-Plane ICP transformation matrix:" << std::endl;
    std::cout << icp_transformation << std::endl;

    // 计算总变换矩阵（4PCS + 点到面ICP）
    Eigen::Matrix4f total_transform = lm_icp.getFinalTransformation();
    ...
    std::cout << "Registration result saved to aligned_cloud.pcd" << std::endl;
    ...
    std::cout << "Final fitness score: " << lm_icp.getFitnessScore() << std::endl;

    // 质量评估
    if (lm_icp.getFitnessScore() < 0.001f)
    {
        std::cout << "Registration quality: Excellent" << std::endl;
    }
    else if (lm_icp.getFitnessScore() < 0.005f)
    {
        std::cout << "Registration quality: Very Good" << std::endl;
    }
    else if (lm_icp.getFitnessScore() < 0.01f)
    {
        std::cout << "Registration quality: Good" << std::endl;
    }
    else if (lm_icp.getFitnessScore() < 0.05f)
    {
    ...
    std::string final_score = "ICP Score: " + std::to_string(lm_icp.getFitnessScore());
