...
int main()
{
    ...
    std::cout << "=== K-4PCS Point Cloud Registration ===" << std::endl;
    ...
    // 可视化
    pcl::visualization::PCLVisualizer::Ptr viewer(new pcl::visualization::PCLVisualizer("K-4PCS Point Cloud Registration"));
    ...
    std::string fitness_info = "Fitness: " + std::to_string(kfpcs.getFitnessScore());
    viewer->addText(fitness_info, 10, 80, 14, 1, 1, 1, "fitness_text", v3);

    // 公共设置
    viewer->addText("K-4PCS Point Cloud Registration", 400, 20, 18, 1, 1, 1, "title");
